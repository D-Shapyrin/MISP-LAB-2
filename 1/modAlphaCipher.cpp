...
{ 
    ...
    for (auto& c : tmp) {
        if (!isalpha(c))
            throw cipher_error(std::string("Invalid key ") + std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(ws));
        if (islower(c))
            c = toupper(c);
    }
    return tmp;
...
{ 
    ...
    for (auto c : s) {
        if (!isupper(c))
            throw cipher_error(std::string("Invalid cipher text ") + std::wstring_convert<std::codecvt_utf8<wchar_t>>().to_bytes(s));
    }
    return s;
}
